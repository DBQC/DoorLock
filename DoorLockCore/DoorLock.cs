...
public class DoorLock
{
    ...
    public DoorLock()
    {
        ...
        //Check the setting is actually a number
        ...
        int.TryParse(ConfigurationManager.AppSettings.AllKeys
                                                     .Where(key => key.StartsWith("LockOutTime"))
                                                     .Select(key => ConfigurationManager.AppSettings[key])
                                                     .ToArray()[0], out _LockOutTime);
        //int.TryParse(DoorLockCore.Properties.Settings.Default["LockOutTime"].ToString(), out _LockOutTime);

    }

    /// <summary>
    /// Returns true if the submitted code is a valid code, false if the code is incorrect.
    /// </summary>
    /// <param name="codeAttempt">The code attempt to evaluate</param>
    /// <returns></returns>
